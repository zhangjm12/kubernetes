...
function provision-master() {
  ...
  kube-scp ${MASTER} "${ROOT}/binaries/master ${ROOT}/master ${ROOT}/node ${ROOT}/config-default.sh ${ROOT}/util.sh" "${KUBE_TEMP}"
  kube-ssh "${MASTER}" " \
    ...
    sudo bash ${KUBE_TEMP}/master/scripts/make-ca-cert.sh ${master_ip} IP:${master_ip},IP:${SERVICE_CLUSTER_IP_RANGE%.*}.1,DNS:kubernetes,DNS:kubernetes.default,DNS:kubernetes.default.svc,DNS:kubernetes.default.svc.cluster.local; \
